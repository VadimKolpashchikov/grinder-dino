...
function HeaderSliderWithPresent({ headerList, setHeaderList }) {
  ...
  return (
    <div className="header-hero__block-image">
      <a
        data-fancybox="video-header"
        href="https://youtu.be/HRc_DCjKkg8"
        className="header-hero__video-btn header-hero__video-btn_notest"
      >
        <svg
          ...
          {headerList.map((item) => (
            <SwiperSlide key={item.id}>
              <Image
                srcImg={item.img}
                srcSource={item.webp}
                srcImgMob={item.imgDesk}
                srcSourceMob={item.webpDesk}
                media={440}
                mobil={true}
                altImg={`ГРИНДЕР DINO ${item.name}`}
              />
            </SwiperSlide>
          ))}
        </Swiper>
...
export default HeaderSliderWithPresent;
